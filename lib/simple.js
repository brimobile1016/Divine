...
return result
  ...
  pack: "Saipul", // The pack name
  author: "Anuar", // The author name
...
const bio = (conn.fetchStatus(num).catch(console.error) || {}).status || '-'
conn.sendMessage(
update.id, 
{ 
image: { url: ppuser },
...
const bio = (conn.fetchStatus(num).catch(console.error) || {}).status || '-'
var ngen = `*Leave From Grup ${metadata.subject}*
...
conn.sendMessage(update.id, { image: { url: ppuser }, caption: ngen, mentions: [num]},{ quoted: {
 key: {
 fromMe: false,
...
return await conn.sendMessage(jid, { [`${pase}`]: data, mimetype: mime, ...options }, { quoted })
  ...
  */
  ...
  +"item2.EMAIL;type=INTERNET:sanuar@asu.edu\n"
  +"item2.X-ABLabel:Email\nitem3.URL:https://youtube.com/@saipulanuar\n"
  +"item3.X-ABLabel:Youtube\n"
  +"item4.ADR:;;Batam,Indonesia;;;;\n"
...
return await conn.sendMessage(jid, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `lana wibu`,
body: `lana sad`,
previewType:"PHOTO",
